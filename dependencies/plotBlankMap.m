function plotBlankMap(figct,region,centeredon,shadingcolor,labels,varargin)
...
%Example usage: plotBlankMap(1,'world',0,'ghost white',0,0);
%plotBlankMap(1,'world',0,'ghost white',0,{'stateboundaries';0}); %with no US states
    
%Runtime: about 1 min for world, proportionally less for individual regions
%Dependencies: colors.m
...
elseif strcmp(region,'worldnorthof60s')
    if centeredon==0
        southlat=-65;northlat=90;westlon=-180;eastlon=180;mapproj='robinson';conttoplot='all';
    elseif centeredon==180
        southlat=-65;northlat=90;westlon=-360;eastlon=0;mapproj='robinson';conttoplot='all';
    end
...
elseif strcmp(region,'world60s60n_mainlandareasonly_mercator')
    southlat=-60;northlat=60;westlon=-130;eastlon=160;mapproj='mercator';conttoplot='all';
...
%disp('line 145');disp(length(varargin{1}));disp(varargin);

if ~strcmp(region, 'centeredonpoint')
    for count=1:2:length(varargin{1})-1
        val=varargin{1};%disp(val);
        if strcmp(val{count},'stateboundaries') 
            stateboundaries=val{2};
        end
    end
end
